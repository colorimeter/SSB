document.addEventListener('DOMContentLoaded', () => {
    ...
    const drawdata = async () => {
        ...
        try {
            ...
            if (!valasz.ok) {
                throw new Error(`HTTP error! ${valasz.status}`);
            }
            ...
            const varosba_str = varosba.map(item => {
             const c=item.countdown.padStart(3, ' ');
             const h=item.dateTime.hour.padStart(2, '0');
             const m=item.dateTime.minute.padStart(2, '0');
             return `${c} perc múlva (${h}:${m}) -> ${item.servingLine.direction}`;
              });
            const kifele_str = kifele.map(item => `${item.countdown} perc múlva (${item.dateTime.hour}:${item.dateTime.minute}) -> ${item.servingLine.direction}`);
            ...
            eredmenyKontener1.innerHTML = `<p style="color: red;">ERROR: ${e.message}</p>`;
    ...
    gomb.addEventListener('click', drawdata);
});
